...
import { getFolders, getFiles } from '../../Contexts/actionCreators/fileFolderActionCreator';
...
const MainLayout = ({ category }) => {
  ...
  const { isLoading, userId } = useSelector(
    (state) =>
    ({
  ...
  useEffect(() => {
    if (userId && category) {
      dispatch(getFolders( category));
      dispatch(getFiles( category));
    }
