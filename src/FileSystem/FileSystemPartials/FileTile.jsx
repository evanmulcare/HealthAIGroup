...
import { FaFolder, FaFile, FaCircle, FaFilePdf, FaFileWord, FaFileExcel, FaFileImage, FaFilePowerpoint, FaRegEdit, FaTimesCircle, FaDownload } from 'react-icons/fa';
import { doc, getDoc } from 'firebase/firestore';
import { db } from '../../firebase';
import { format } from 'date-fns'; 

const FileTile = ({ name, type, onDoubleClick, UserId, updatedAt, setIsDeleteModalOpen, setIsEditModalOpen, url, setFileName, setURL }) => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userDocSnapshot.exists()) {
          setUserData({ id: userDocSnapshot.id, ...userDocSnapshot.data() });
        } else {
          ...
          {showEditMenu && (
            <div className="relative">
              <div className="absolute top-full w-44 left-20 mt-2 bg-white border border-gray-300 rounded-md shadow-md z-10 text-gray-700">
                <ul>
                  <li className="px-4 w-full py-2 cursor-pointer hover:bg-gray-100 inline-flex" onClick={onDoubleClick}
                  >
                    <FaRegEdit className="text-md mr-2 " />
                    <span className="inline text-sm mb-2">Edit</span>
                  </li>
                  ...
                  <li className="px-4 w-full py-2 cursor-pointer hover:bg-gray-100 inline-flex" onClick={() => { setIsDeleteModalOpen(true); setURL(url); setFileName(name); }}
                  >
                    <FaTimesCircle className="text-md mr-2 text-red-500 " /> 
                    <span className="inline text-sm mb-2">Delete</span>
                  </li>
