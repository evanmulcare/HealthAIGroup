...
import { uploadFile } from '../../Contexts/actionCreators/fileFolderActionCreator';
...
const UploadFileModal = ({ setIsUploadFileModalOpen, category }) => {
  ...
  const { userFiles, user, currentFolder, currentFolderData } = useSelector((state) => ({
    ...
    currentFolderData: state.filefolders.userFiles.find((folder) => folder.docId == state.filefolders.currentFolder),
  }), shallowEqual);
  ...
  const checkFileAlreadyPresent = (name) => {
    const filePresent = userFiles.find((file) => file.parent === currentFolder && file.name === name);
  ...
  const HandleSubmit = (e) => {
    ...
    if (file) {
      if (!checkFileAlreadyPresent(file.name)) {
        ...
        dispatch(uploadFile(file, data,setSuccess,category ));
      } else {
