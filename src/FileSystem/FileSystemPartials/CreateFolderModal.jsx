...
import { createFolder } from '../../Contexts/actionCreators/fileFolderActionCreator';
...
const currentUser = useSelector((state) => state.auth.currentUser);
    ...
    const checkFolderAlreadyPresent = (name) => {
      const folderPresent = userFolders.find((folder) => folder.parent === currentFolder && folder.name === name);
        ...
        if(folderName) 
        {
            if(folderName.length > 3)
            {
                if(!checkFolderAlreadyPresent(folderName))
                {
                    ...
                    dispatch(createFolder(data, category));
                }
                else{
